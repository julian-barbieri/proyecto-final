...
function getStatusContent(recursa) {
  if (recursa) {
    return {
      border: "border-orange-200",
      accent: "text-orange-700",
      soft: "bg-orange-50",
      title: "RECURSARÁ LA MATERIA",
      subtitle:
        "El modelo estima alta probabilidad de recursado para esta materia.",
    };
  }
  ...
  return {
    ...
    title: "NO RECURSARÁ",
    subtitle:
      "El modelo estima que el alumno podría aprobar la materia sin recursar.",
  };
}
...
export default function RecursadoResult({ result }) {
  ...
  const status = getStatusContent(Boolean(result.Recursa));
    ...
    >
      ...
      <div className="mt-6 space-y-3">
        ...
        <p className="text-sm text-slate-600">
          Probabilidad estimada de recursado: {percentage}%
        </p>
      </div>
    </section>
  );
